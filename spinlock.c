...
{
    int n_sem, i;
    if(argint(0, &n_sem) < 0) return -1;

    // find semaphore that is not be allocated
    for(i = 0; i < SEM_MAX_NUM; i++) {
        acquire(&sems[i].lock);
...
sys_sem_p()
{
    ...
    // lack of resources, release spinlock and block the process
    if(sems[id].resource_count < 0)
        sleep(&sems[id], &sems[id].lock);
    // have reacquired spinlock for a while and now release it
    release(&sems[id].lock);
    return 0;
}
...
sys_sem_v()
{
    ...
    // if there are blocked processes, wakeup 1 process
    if(sems[id].resource_count <= 0)
        wakeup1p(&sems[id]);
    release(&sems[id].lock);
